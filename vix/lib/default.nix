...
rec
  ...
  nivGoModule =
    ...
    let
      ...
      meta = { inherit name; description = name; version = src.version or src.rev; } // src;
    in
    (
      pkgs.buildGoModule
        (
                ...
                // {
                  ...
                  overrideModAttrs =
                    old:
                    {
                      buildPhase =
                        ''
                        go mod init ${ meta.go_module or "github.com/${ meta.owner }/${ meta.repo }" }
                        ${ old.buildPhase }
                        '';
                      installPhase =
                        ''
  ...
  nivFishPlugin = name: { inherit name; src = nivSources."fish-${ name }"; };
  vixLink = path: lib.mds.mkOutOfStoreSymlink "/hk/vix/${ path }";
  nivApp = name: lib.mds.installNivDmg { inherit name; src = nivSources."${ name }App"; };
}
